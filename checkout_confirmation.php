...
// «аполенные пол€ методов оплаты(если есть)
if (is_array($payment_modules->modules))
{
...
// ≈сли метод оплаты требует перехода на сайт сервиса, то посылаем на form_action_url,
// в противном случае форма отправл€ет на окончательное формирование заказа
...
//  нопка подтверждени€ заказа
$_array = array
(
	'img' => 'button_confirm_order.gif',
	'href' => '',
	'alt' => TEXT_BUTTON_CONFIRM_ORDER,
	'code' => ''
);
...
if (DISPLAY_REVOCATION_ON_CHECKOUT == 'true')
{
	...
	$shop_content_query = os_db_query("SELECT content_title, content_heading, content_text, content_file FROM ".TABLE_CONTENT_MANAGER." WHERE content_group='".REVOCATION_ID."' ".$group_check." AND languages_id='".$_SESSION['languages_id']."'");
	...
	$shop_content_query = os_db_query("SELECT content_title, content_heading, content_text, content_file FROM ".TABLE_CONTENT_MANAGER." WHERE content_group='3' ".$group_check." AND languages_id='".$_SESSION['languages_id']."'");
	$shop_content_data = os_db_fetch_array($shop_content_query);
