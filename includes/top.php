<?php
    ...
    */
    ...
    os_db_connect() or die('Unable to connect to database server!'); //подключение к бд
    ...
    $p = new plugins(); // получения списка активных плагинов

    $p->require_plugins(); //подключение активных плагинов

    get_cache_all(); //читаем кэш

    remove_action_array (); //удаляем action плагинов

    require_once (_LIB . 'phpmailer/class.phpmailer.php');
    if (EMAIL_TRANSPORT == 'smtp') require_once (_LIB . 'phpmailer/class.smtp.php');
    ...
    session_name('sid');
    if (STORE_SESSIONS != 'mysql') session_save_path(_CATALOG.SESSION_WRITE_DIRECTORY);
    ...
    elseif (isset($_GET['manufacturers_id']) && os_not_null($_GET['manufacturers_id'])) {
        $manufacturers_query = osDBquery("select manufacturers_name from ".TABLE_MANUFACTURERS." where manufacturers_id = '".(int) $_GET['manufacturers_id']."'");
    ...
    if (isset($tPath_array)) {
        ...
        $authors_query = osDBquery("select authors_name from " . TABLE_AUTHORS . " where authors_id = '" . (int)$_GET['authors_id'] . "'");
        if (os_db_num_rows($authors_query,true)) {
            $authors = os_db_fetch_array($authors_query,true);
            $breadcrumb->add(ARTICLES_BY . $authors['authors_name'], os_href_link(FILENAME_ARTICLES, 'authors_id=' . $_GET['authors_id']));
        }
    }

    // add the articles name to the breadcrumb trail
    if (isset($_GET['articles_id'])) {
		$article_query = osDBquery("select articles_name from ".TABLE_ARTICLES_DESCRIPTION." where articles_id = '".( int )$_GET['articles_id']."' and language_id = '" . (int)$_SESSION['languages_id'] . "'");

        if (os_db_num_rows($article_query,true)) {
            ...
            if (isset($_GET['authors_id'])) {
                $breadcrumb->add($article['articles_name'], os_href_link(FILENAME_ARTICLE_INFO, 'authors_id=' . $_GET['authors_id'] . '&articles_id=' . $_GET['articles_id'] . $SEF_parameter));
            } else {
    ...
    if (isset($_GET['news_id'])) {
        $news_query = osDBquery("select news_id, headline from " . TABLE_LATEST_NEWS . " where news_id = '" . (int)$_GET['news_id'] . "' and language = '" . (int)$_SESSION['languages_id'] . "'");
        if (os_db_num_rows($news_query,true)) {
            $news = os_db_fetch_array($news_query,true);

            $SEF_parameter = '';
            if (SEARCH_ENGINE_FRIENDLY_URLS == 'true')
                $SEF_parameter = '&headline='.os_cleanName($news['headline']);
