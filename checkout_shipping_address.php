...
if (isset ($_POST['action']) && ($_POST['action'] == 'submit')) {
   ...
   if (ACCOUNT_CITY == 'true') {
		...
		if ($error == false) {
			...
			$_SESSION['sendto'] = os_db_insert_id();
	...
	elseif (isset ($_POST['address'])) {
		...
		$check_address_query = os_db_query("select count(*) as total from ".TABLE_ADDRESS_BOOK." where customers_id = '".$_SESSION['customer_id']."' and address_book_id = '".$_SESSION['sendto']."'");
...
$breadcrumb->add(NAVBAR_TITLE_1_CHECKOUT_SHIPPING_ADDRESS, os_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL'));
$breadcrumb->add(NAVBAR_TITLE_2_CHECKOUT_SHIPPING_ADDRESS, os_href_link(FILENAME_CHECKOUT_SHIPPING_ADDRESS, '', 'SSL'));
...
if ($process == false) {
	...
	if ($addresses_count > 1) {
		...
		while ($addresses = os_db_fetch_array($addresses_query)) {
			...
			$address_content .= '<p><span class="bold">'.$addresses['firstname'].' '.$addresses['secondname'].' '.$addresses['lastname'].'</span>&nbsp;'.os_draw_radio_field('address', $addresses['address_book_id'], ($addresses['address_book_id'] == $_SESSION['sendto'])).'</p>
...
$main_content = $osTemplate->fetch(CURRENT_TEMPLATE.'/module/checkout_shipping_address.html');

$osTemplate->assign('language', $_SESSION['language']);
$osTemplate->assign('main_content', $main_content);
$osTemplate->caching = 0;
 $osTemplate->loadFilter('output', 'trimhitespace');
$template = (file_exists(_THEMES_C.FILENAME_CHECKOUT_SHIPPING_ADDRESS.'.html') ? CURRENT_TEMPLATE.'/'.FILENAME_CHECKOUT_SHIPPING_ADDRESS.'.html' : CURRENT_TEMPLATE.'/index.html');
