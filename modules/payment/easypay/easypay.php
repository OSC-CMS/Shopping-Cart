...
if ($_SERVER["HTTP_X_FORWARDED_FOR"]) {
    ...
    function process_button() {
      ...
      $process_button_string = '';
			...
			$ep["success"]		= os_href_link(FILENAME_CHECKOUT_PROCESS);
			$ep["cancel"]		= os_href_link(FILENAME_CHECKOUT_CONFIRMATION);
			$ep["debug"]		= MODULE_PAYMENT_EASYPAY_DEBUG;

			#подготовка формы для оплаты
			$process_button_string =
			os_draw_hidden_field("EP_MerNo",			$ep["merno"]).
			os_draw_hidden_field("EP_OrderNo",			$ep["oderno"]).
			os_draw_hidden_field("EP_Sum",				$ep["sum"]).
			os_draw_hidden_field("EP_Expires",			$ep["expiries"]).
			os_draw_hidden_field("EP_Comment",			$ep["comment"]).
			os_draw_hidden_field("EP_OrderInfo",		$ep["orderinfo"]).
			os_draw_hidden_field("EP_Hash",			$ep["hash"]).
			os_draw_hidden_field("EP_Success_URL",		$ep["success"]).
			os_draw_hidden_field("EP_Cancel_URL",		$ep["cancel"]).
			os_draw_hidden_field("EP_Debug",			$ep["debug"]);
