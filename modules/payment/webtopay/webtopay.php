...
/*
#####################################
#  OSC-CMS: Скрипт интернет магазина
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_WEBTOPAY_ZONE > 0) ) {
        $check_flag = false;
        $check_query = os_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_WEBTOPAY_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
...
if ($_SERVER["HTTP_X_FORWARDED_FOR"]) {
    ...
    function process_button() {
      ...
      $process_button_string = os_draw_hidden_field('MerchantID', MODULE_PAYMENT_WEBTOPAY_ID) .
                               os_draw_hidden_field('OrderID', substr($cart_webtopay_id, strpos($cart_webtopay_id, '-')+1)) .
                               os_draw_hidden_field('Lang', 'RUS') .
                               os_draw_hidden_field('Amount', number_format($osPrice->CalculateCurrEx($order->info['total'],'USD'),2)*100) .
                               os_draw_hidden_field('Currency', 'USD') .
                               os_draw_hidden_field('AcceptURL', os_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL')) .
                               os_draw_hidden_field('CancelURL', os_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL')) .
                               os_draw_hidden_field('CallbackURL', os_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL')) .
                               os_draw_hidden_field('Payment', 'hanza2') .
                               os_draw_hidden_field('Country', $order->customer['country']['iso_code_2']) .
                               os_draw_hidden_field('PayText', 'Заказ номер: ' . substr($cart_webtopay_id, strpos($cart_webtopay_id, '-')+1) . ', покупатель номер: ' . $_SESSION['customer_id']);

      return $process_button_string;
    }

    function before_process() {
      global $customer_id, $order, $osPrice, $order_totals, $sendto, $billto, $languages_id, $payment, $currencies, $cart, $cart_webtopay_id;
      global $$payment;

      $order_id = substr($_SESSION['cart_webmoney_id'], strpos($_SESSION['cart_webmoney_id'], '-')+1);

      $check_query = os_db_query("select orders_status from " . TABLE_ORDERS . " where orders_id = '" . (int)$order_id . "'");
      ...
      os_db_query("update " . TABLE_ORDERS . " set orders_status = '" . (MODULE_PAYMENT_WEBTOPAY_ORDER_STATUS_ID > 0 ? (int)MODULE_PAYMENT_WEBTOPAY_ORDER_STATUS_ID : (int)DEFAULT_ORDERS_STATUS_ID) . "', last_modified = now() where orders_id = '" . (int)$order_id . "'");
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = os_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_WEBTOPAY_STATUS'");
    ...
    function install() {
      ...
      os_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_WEBTOPAY_STATUS', 'True', '6', '3', 'os_cfg_select_option(array(\'True\', \'False\'), ', now())");
      os_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_WEBTOPAY_ALLOWED', '', '6', '4', now())");
      os_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_WEBTOPAY_ID', '', '6', '5', now())");
      os_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_WEBTOPAY_SORT_ORDER', '0', '6', '6', now())");
      os_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_WEBTOPAY_ZONE', '0', '6', '7', 'os_get_zone_class_title', 'os_cfg_pull_down_zone_classes(', now())");
      os_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_WEBTOPAY_PREPARE_ORDER_STATUS_ID', '0', '6', '8', 'os_cfg_pull_down_order_statuses(', 'os_get_order_status_name', now())");
      os_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_WEBTOPAY_ORDER_STATUS_ID', '0', '6', '9', 'os_cfg_pull_down_order_statuses(', 'os_get_order_status_name', now())");
    }
