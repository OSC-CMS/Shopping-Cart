...
class roboxchange extends OscCms
{
	...
	function process_button()
	{
		...
		$crc  = md5(MODULE_PAYMENT_ROBOXCHANGE_LOGIN.':'.$order_sum.':'.substr($_SESSION[$this->name], strpos($_SESSION[$this->name], '-')+1).':'.MODULE_PAYMENT_ROBOXCHANGE_PASSWORD1);

		$process_button_string = 
			os_draw_hidden_field('InvId', substr($_SESSION[$this->name], strpos($_SESSION[$this->name], '-')+1)) .
			os_draw_hidden_field('MrchLogin', MODULE_PAYMENT_ROBOXCHANGE_LOGIN) .
			os_draw_hidden_field('Desc', substr($_SESSION[$this->name], strpos($_SESSION[$this->name], '-')+1)) .
