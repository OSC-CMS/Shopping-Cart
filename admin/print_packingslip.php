	...
	$osTemplate->assign('company_accountant', $company['accountant']);
  	...
  	$osTemplate->assign('csID',$order->customer['csID']);
        ...
        $order_data[]=array(
        		...
        		'PRODUCTS_SINGLE_PRICE' =>  os_format_price_order($order_data_values['products_price'],1,$order->info['currency']),
        		'PRODUCTS_PRICE' =>  os_format_price_order($order_data_values['final_price'],1,$order->info['currency']),
...
if ($order->info['shipping_class'] != '') {
  ...
  $order_shipping_text = ((substr($shipping_method['title'], -1) == ':') ? substr(strip_tags($shipping_method['title']), 0, -1) : strip_tags($shipping_method['title']));
  	...
  	$osTemplate->display(CURRENT_TEMPLATE . '/module/print_packingslip.html');	
