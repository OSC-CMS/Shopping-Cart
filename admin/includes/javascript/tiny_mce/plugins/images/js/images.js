...
var images_onload = function() {
	...
	function showFootInfo() {
		...
		if(file.length > 1) {
			...
			$('#footExt').text(tinyMCEPopup.getLang('images_dlg.files_selected') + ': '+file.length);
			var tmpSizeCount = 0;
			...
			$('#footExt').text(tinyMCEPopup.getLang('images_dlg.files_total') + ': '+allFiles.length);
			var tmpSizeCount = 0;
	...
	function intToMb(i) {
		if(i < 1024) {
			return i + ' ' + tinyMCEPopup.getLang('images_dlg.bytes');
		} else if(i < 1048576) {
			var v = i/1024;
			v = parseInt(v*10)/10;
			return v + ' ' + tinyMCEPopup.getLang('images_dlg.kbytes');
		} else {
			var v = i/1048576;
			v = parseInt(v*10)/10;
			return v + ' ' + tinyMCEPopup.getLang('images_dlg.mbytes');
		}
	}
	...
	$('#fileOpen').MultiFile({ 
		STRING: { 
			remove:'<img src="img/cross_small.png" width="16" height="16" alt="' + tinyMCEPopup.getLang('images_dlg.remove') + '" />',
			denied:tinyMCEPopup.getLang('images_dlg.you_can_not_upload'),
			duplicate:tinyMCEPopup.getLang('images_dlg.file_already_added')
		},
		max: 5,
	...
	swfu = new SWFUpload({
        ...
        button_image_url: "img/spacer.gif",
		post_params: {
			//"PHPSESSID" : "NONE",
			"action" : "uploadfile",
		},
		...
		file_types : "*.*",
		file_types_description : tinyMCEPopup.getLang('images_dlg.all_files'),
...
// this plugin events must be attached in queue AFTER tinyMCEPopup._onDOMLoaded, that translates i18n, replacing innerHTML
if (window.attachEvent) window.attachEvent('onload', images_onload); // IE
else if (window.addEventListener) // modern
	window.addEventListener('load', function() {
		images_onload.call(document); // `$(this).keyup` above crashes TinyMCE-under-Chrome on Esc, when `this==window` and not `document`: http://tinymce.moxiecode.com/develop/bugtracker_view.php?id=2817
	}, false);
