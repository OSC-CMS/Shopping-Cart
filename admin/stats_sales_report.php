...
{    $srView = $_GET['report'];
  ...
  if ( ($_GET['detail']) && (os_not_null($_GET['detail'])) )
{    $srDetail = $_GET['detail'];
  ...
  if ( ($_GET['export']) && (os_not_null($_GET['export'])) )
{    $srExp = $_GET['export'];
  }
  if ($srExp < 0 || $srExp > 2) {
    $srExp = $srDefaultExp;
  ...
  if ( ($_GET['max']) && (os_not_null($_GET['max'])) ) {
    $srMax = $_GET['max'];
  }
  if (!is_numeric($srMax)) {
    $srMax = 0;
  ...
  if ( ($_GET['status']) && (os_not_null($_GET['status'])) )
{    $srStatus = $_GET['status'];
  ...
  if ( ($_GET['payment']) && (os_not_null($_GET['payment'])) )
{    $srPayment = $_GET['payment'];
  } else {
 	$srPayment = 0;
  }
  
  if ( ($_GET['sort']) && (os_not_null($_GET['sort'])) ) {
    $srSort = $_GET['sort'];
  }
...
{    $sYear = $_GET['startY'];
  ...
  require(get_path('class_admin') . 'sales_report.php');
...
<?php
  ...
  require(_MODULES.'payment/'.str_replace('.php','',$payments[$i]).'/'.$_SESSION['language'].'.php');	
...
<?php
    switch ($srView) {
      case '3':
?>
                      <td class="dataTableContent" align="right"><?php echo os_date_long(date("Y-m-d\ H:i:s", $sr->showDate)) . " - " . os_date_short(date("Y-m-d\ H:i:s", $sr->showDateEnd)); ?></td>
<?php
        break;
      case '4':
?>
                      <td class="dataTableContent" align="right"><?php echo os_date_long(date("Y-m-d\ H:i:s", $sr->showDate)); ?></td>
<?php
        break;
      default;
?>
                      <td class="dataTableContent" align="right"><?php echo os_date_short(date("Y-m-d\ H:i:s", $sr->showDate)) . " - " . os_date_short(date("Y-m-d\ H:i:s", $sr->showDateEnd)); ?></td>
<?php
    }
?>
...
?>
                    ...
                    <td class="dataTableContent" align="left"><a href="<?php echo os_catalog_href_link("product_info.php?products_id=" . $info[$i]['pid']) ?>" target="_blank"><?php echo $info[$i]['pmodel'].' : '.$info[$i]['pname']; ?></a>
<?php
