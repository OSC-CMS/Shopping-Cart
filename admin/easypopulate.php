		...
		if (isset($filelayout['v_attribute_options_id_1'])){
			...
			foreach ($attribute_options_array as $attribute_options) {
				...
				for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
					...
					$attribute_options_languages_values = os_db_query($attribute_options_languages_query);
				...
				while ($attribute_values = os_db_fetch_array($attribute_values_values)) {
					...
					$attribute_values_price_values = os_db_query($attribute_values_price_query);
					...
					for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
						...
						$attribute_values_languages_values = os_db_query($attribute_values_languages_query);
...
//      }
	...
	if (trim($theheaders_array[0]) =='v_products_model' &&
    	trim($theheaders_array[1]) =='v_products_price' && 
		trim($theheaders_array[2]) =='v_products_quantity')
	{
	   $os_walk = true;
	}
	else
			...
			<option value="utf8" size="5">utf8</option>
					...
					<td>
					 ...
					 <FORM ENCTYPE="multipart/form-data" ACTION="easypopulate.php" METHOD=POST>
							<p>
								<div align = "left">
				<?php echo os_draw_form('localfile_insert', 'easypopulate.php', '', 'post', 'ENCTYPE="multipart/form-data"'); ?>
									<p>
										<div align = "left">
<span class="smallText">
	...
	$result = os_db_query("SELECT products_id FROM ".TABLE_PRODUCTS." WHERE (products_id = '". $v_products_id . "')");
		...
		if (isset($v_categories_id)){
			...
			if (os_db_num_rows($result_incategory) == 0) {
				...
				$res1 = os_db_query('INSERT INTO '.TABLE_PRODUCTS_TO_CATEGORIES.' (products_id, categories_id)
			...
			while (isset($$v_attribute_options_id_var) && !empty($$v_attribute_options_id_var)) {
					...
					if (os_db_num_rows($attribute_options_values) <= 0) {
						for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
							...
							if (isset($$v_attribute_options_name_var)) {
								...
								$attribute_options_insert = os_db_query($attribute_options_insert_query);
							}
						}
						...
						for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
							...
							if (isset($$v_attribute_options_name_var)) {
								...
								$attribute_options_update_lang_values = os_db_query($attribute_options_update_lang_query);
								...
								if (os_db_num_rows($attribute_options_update_lang_values) <= 0) {
									...
									$attribute_options_lang_insert = os_db_query($attribute_options_lang_insert_query);
									...
									$attribute_options_update = os_db_query($attribute_options_update_query);
								}
							}
						}
					}
				...
				while (isset($$v_attribute_values_id_var) && !empty($$v_attribute_values_id_var)) {
						...
						if (os_db_num_rows($attribute_values_values) <= 0) {
							for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
								...
								if (isset($$v_attribute_values_name_var)) {
									...
									$attribute_values_insert = os_db_query($attribute_values_insert_query);
								}
							}
							...
							$attribute_values_pov2po = os_db_query($attribute_values_pov2po_query);
						} else { // update options table, if options already exists
							for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
								$lid = $languages[$i]['id'];
								...
								if (isset($$v_attribute_values_name_var)) {
									...
									$attribute_values_update_lang_values = os_db_query($attribute_values_update_lang_query);
									...
									if (os_db_num_rows($attribute_values_update_lang_values) <= 0) {
										...
										$attribute_values_lang_insert = os_db_query($attribute_values_lang_insert_query);
										...
										$attribute_values_update = os_db_query($attribute_values_update_query);
									}
								}
							}
						}
					...
					if (isset($$v_attribute_values_price_var) && ($$v_attribute_values_price_var != '')) {
						...
						$attribute_prices_values = os_db_query($attribute_prices_query);
						...
						if (os_db_num_rows($attribute_prices_values) <= 0) {
							...
							$attribute_prices_insert = os_db_query($attribute_prices_insert_query);
						} else { // update options table, if options already exists
							...
							$attribute_prices_update = os_db_query($attribute_prices_update_query);
						}
					}
